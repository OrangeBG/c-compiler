...
static void resolve_file_scope_variable_declaration(char *identifier, enum DeclarationType declaration_type, HashTable *declarations);  
static void resolve_local_scope_variable_declaration(char *identifier, enum DeclarationType declaration_type, HashTable *declarations);   
  ...
  switch (node->type) {
    case AST_VARIABLE_DECLARATION: {
      ...
      if (declaration_stack->count == 1) {
        ...
        resolve_local_scope_variable_declaration(node->data.variable_declaration.name, DECLARATION_TYPE_VARIABLE, declaration_table);   
      }
