...
static IRNode* emit_function(AstNode *ast_function, IREmitStatus *emit_status, Arena *node_arena, DeclarationSymbolTable *declaration_symbol_table) {
  ...
  emit_ast_node(ast_function->data.function_declaration.body_block, function, emit_status, node_arena, declaration_symbol_table);
...
static IRNode* emit_ast_node(AstNode *node, IRNode *function, IREmitStatus *emit_status, Arena *node_arena, DeclarationSymbolTable *declaration_symbol_table) {
  switch (node->type) {
      ...
      case AST_STATEMENT_CONTINUE:           { emit_continue(node->data.continue_statement.label_id, function, node_arena); break; }
      case AST_STATEMENT_BREAK:              { emit_break(node->data.break_statement.label_id, function, node_arena); break; }
      case AST_STATEMENT_COMPOUND:           { emit_block(node->data.compound_statement.block, function, emit_status, node_arena, declaration_symbol_table); break; }
      ...
      case AST_EXPRESSION_VARIABLE:          { return create_variable(node->data.variable_expression.identifier, node_arena); }
...
static IRNode* emit_return(AstNode *block_item, IRNode *function, IREmitStatus *emit_status, Arena *node_arena, DeclarationSymbolTable *declaration_symbol_table) {
  IRNode *value = emit_ast_node(block_item->data.return_statement.expression, function, emit_status, node_arena, declaration_symbol_table);
  IRNode *return_instruction = arena_alloc(node_arena);
...
static void emit_if(AstNode *if_node, IRNode *function, IREmitStatus *emit_status, Arena *node_arena, DeclarationSymbolTable *declaration_symbol_table) {
  IRNode *condition = emit_ast_node(if_node->data.if_statement.condition_expression, function, emit_status, node_arena, declaration_symbol_table);
...
static void emit_goto(AstNode *goto_node, IRNode *function, Arena *node_arena) {
  emit_jump(goto_node->data.goto_label_statement.label, function, node_arena);
}

static void emit_goto_label(AstNode *goto_label_node, IRNode *function, Arena *node_arena) {
  emit_label(goto_label_node->data.goto_statement.label, function, node_arena);
}

static void emit_while(AstNode *while_node, IRNode *function, IREmitStatus *emit_status, Arena *node_arena, DeclarationSymbolTable *declaration_symbol_table) {
  char *continue_label_identifier = create_concat_identifier(CONTINUE_LABEL, while_node->data.do_while_statement.label_id); 
  char *break_label_identifier = create_concat_identifier(BREAK_LABEL, while_node->data.do_while_statement.label_id); 

  emit_label(continue_label_identifier, function, node_arena);

  IRNode *condition = emit_ast_node(while_node->data.while_statement.condition, function, emit_status, node_arena, declaration_symbol_table);

  emit_jump_if_zero(break_label_identifier, condition, function, node_arena);
  emit_ast_node(while_node->data.while_statement.statement_body, function, emit_status, node_arena, declaration_symbol_table);
  emit_jump(continue_label_identifier, function, node_arena);
  emit_label(break_label_identifier, function, node_arena);
}

static void emit_do_while(AstNode *do_node, IRNode *function, IREmitStatus *emit_status, Arena *node_arena, DeclarationSymbolTable *declaration_symbol_table) {
  char *start_label_identifier = create_concat_identifier(START_LABEL, do_node->data.do_while_statement.label_id);
  emit_label(start_label_identifier, function, node_arena);

  emit_ast_node(do_node->data.do_while_statement.statement_body, function, emit_status, node_arena, declaration_symbol_table);

  char *continue_label_identifier = create_concat_identifier(CONTINUE_LABEL, do_node->data.do_while_statement.label_id); 
  emit_label(continue_label_identifier, function, node_arena);

  IRNode *condition = emit_ast_node(do_node->data.do_while_statement.condition, function, emit_status, node_arena, declaration_symbol_table);
  emit_jump_if_not_zero(start_label_identifier, condition, function, node_arena);

  char *break_label_identifier = create_concat_identifier(BREAK_LABEL, do_node->data.do_while_statement.label_id);
  emit_label(break_label_identifier, function, node_arena);
}

static void emit_for(AstNode *for_node, IRNode *function, IREmitStatus *emit_status, Arena *node_arena, DeclarationSymbolTable *declaration_symbol_table) {
  if (for_node->data.for_statement.for_loop_init != NULL) {
    emit_ast_node(for_node->data.for_statement.for_loop_init, function, emit_status, node_arena, declaration_symbol_table);
  }  

  char *start_label_identifier = create_concat_identifier(START_LABEL, for_node->data.for_statement.label_id);
  emit_label(start_label_identifier, function, node_arena);

  char *break_label_identifier = create_concat_identifier(BREAK_LABEL, for_node->data.for_statement.label_id);

  if (for_node->data.for_statement.condition_expression != NULL) {
    IRNode *condition = emit_ast_node(for_node->data.for_statement.condition_expression, function, emit_status, node_arena, declaration_symbol_table);
    emit_jump_if_zero(break_label_identifier, condition, function, node_arena);
  }

  emit_ast_node(for_node->data.for_statement.statement_body, function, emit_status, node_arena, declaration_symbol_table);

  char *continue_label_identifier = create_concat_identifier(CONTINUE_LABEL, for_node->data.for_statement.label_id);
  emit_label(continue_label_identifier, function, node_arena);

  if (for_node->data.for_statement.post_expression != NULL) {
    emit_ast_node(for_node->data.for_statement.post_expression, function, emit_status, node_arena, declaration_symbol_table);
  }
...
static IRNode* emit_declaration(AstNode *declaration_node, IRNode *function, IREmitStatus *emit_status, Arena *node_arena, DeclarationSymbolTable *declaration_symbol_table) {
  ...
  IRNode *node = emit_ast_node(declaration_node->data.variable_declaration.init_expression, function, emit_status, node_arena, declaration_symbol_table);    
  add_postfix_operations(function, emit_status, node_arena, declaration_symbol_table);
...
static IRNode* emit_conditional_expression(AstNode *conditional_node, IRNode *function, IREmitStatus *emit_status, Arena *node_arena, DeclarationSymbolTable *declaration_symbol_table) {
  IRNode *condition = emit_ast_node(conditional_node->data.conditional_expression.condition, function, emit_status, node_arena, declaration_symbol_table);
  ...
  IRNode *true_value = emit_ast_node(conditional_node->data.conditional_expression.true_expression, function, emit_status, node_arena, declaration_symbol_table);

  emit_jump(end_label_name, function, node_arena);
  emit_label(false_label_name, function, node_arena);

  IRNode *false_value = emit_ast_node(conditional_node->data.conditional_expression.false_expression, function, emit_status, node_arena, declaration_symbol_table);      
...
static IRNode* emit_unary_expression(AstNode *unary_node, IRNode *function, IREmitStatus *emit_status, Arena *node_arena, DeclarationSymbolTable *declaration_symbol_table) {
  IRNode *source = emit_ast_node(unary_node->data.unary_expression.expression, function, emit_status, node_arena, declaration_symbol_table);
...
static IRNode* emit_binary_expression(AstNode *binary_node, IRNode *function, IREmitStatus *emit_status, Arena *node_arena, DeclarationSymbolTable *declaration_symbol_table) {
  IRNode *source_1 = emit_ast_node(binary_node->data.binary_expression.left_expression, function, emit_status, node_arena, declaration_symbol_table);
  IRNode *source_2 = emit_ast_node(binary_node->data.binary_expression.right_expression, function, emit_status, node_arena, declaration_symbol_table);
  ...
  add_automatic_variable_declaration_symbol(declaration_symbol_table, binary_node->data.binary_expression.expression_type->data.type.type, destination_name);
...
static IRNode* emit_assignment_expression(AstNode *assignment_node, IRNode *function, IREmitStatus *emit_status, Arena *node_arena, DeclarationSymbolTable *declaration_symbol_table) {
  ...
  if (assignment_node->data.assignement_expression.right_expression->type == AST_EXPRESSION_CONDITIONAL) {
    IRNode *condition = emit_ast_node(assignment_node->data.assignement_expression.right_expression->data.conditional_expression.condition, function, emit_status, node_arena, declaration_symbol_table);
    ...
    IRNode *true_value = emit_ast_node(assignment_node->data.assignement_expression.right_expression->data.conditional_expression.true_expression, function, emit_status, node_arena, declaration_symbol_table);
    ...
    IRNode *false_value = emit_ast_node(assignment_node->data.assignement_expression.right_expression->data.conditional_expression.false_expression, function, emit_status, node_arena, declaration_symbol_table);
  ...
  IRNode *result = emit_ast_node(assignment_node->data.assignement_expression.right_expression, function, emit_status, node_arena, declaration_symbol_table);
...
static IRNode* emit_cast_expression(AstNode *cast_node, IRNode *function, IREmitStatus *emit_status, Arena *node_arena, DeclarationSymbolTable *declaration_symbol_table) {
  IRNode *cast_expression = emit_ast_node(cast_node->data.cast_expression.expression, function, emit_status, node_arena, declaration_symbol_table);
  Types expression_type = get_node_type(cast_expression, declaration_symbol_table); 
  Types target_type = cast_node->data.cast_expression.target_type->data.type.type;
